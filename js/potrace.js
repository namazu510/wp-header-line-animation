...
Potrace = (function() {
    ...
    var adjustVertices = function(pt, sums, po) {
        ...
        for (i = 0; i < m; i++) {
            ...
            minCoord.x = s.x;
            minCoord.y = s.y; // coordinates of minimum
            min = Q.apply(s);
    ...
    Curve.prototype = {
        ...
        toString: function() {
            return ["alpha0: ", this.alpha0, LF, "alpha:  ", this.alpha, LF, "beta:   ", this.beta, LF, "corner: ", (this.tag == POTRACE_CORNER), LF, "bezier: ", this.c[0], COMMA, this.c[1], COMMA, this.c[2]].join(NULL_STR);
        },
        tag: 0,
        ...
        outpath: function() {
            ...
            for (var i = 0, len = $a.length; i < len; i++) {
                ...
                if (c.tag == POTRACE_CORNER) {
                    ...
                    pathtextbits.push([C, cc[0].x, SPACE, cc[0].y, SPACE, cc[1].x, SPACE, cc[1].y, SPACE, cc[2].x, SPACE, cc[2].y, SPACE].join(NULL_STR));
                    var cnt = 0;
                }
                pt = cc[2];
            }
    ...
    var setParam = function(_param) {
        if (_param.threshold != undefined) {
            param.threshold = _param.threshold - 0;
        }
        if (_param.turdSize != undefined) {
            param.turdSize = _param.turdSize - 0;
        }
        if (_param.turnPolicy != undefined) {
            param.turnPolicy = _param.turnPolicy - 0;
        }
        if (_param.alphamax != undefined) {
            param.alphamax = _param.alphamax - 0;
        }
        if (_param.precision != undefined) {
